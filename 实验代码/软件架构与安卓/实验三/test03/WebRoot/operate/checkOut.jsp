<%@ page language="java" import="java.util.*" import="com.entity.House" pageEncoding="utf-8"%>
<%
     ...
     <table border="1" cellspacing="0" align="center">
		...
		<%
			...
			System.out.println(house.getHouseState().replaceAll(" ", ""));
				if(house.getHouseState().replaceAll(" ", "").equals("已入住"))
				{
		%>
		<tr>
     		<td><%=house.getHouseId() %></td><td><%=house.getHouseState() %></td>
     		<td><%=house.getHousePrice() %></td><td><%=house.getHouseDiscount() %></td>
     		<td><%=house.getHouseDeposit() %></td><td><a href="checkOutDelServlet?houseId=<%=house.getHouseId() %>">退房 </a></td>
		</tr>
		<%		}
			}
		 %>
